...
| Reconnecting a **browser** to an in-flight turn (live lane → durable replay, cursor continuation, page-load discovery) | `web-react` — `followTurn` / `followDurableTurn` / `useTurnRecovery` / `chatStreamLaneListener`; the live lane is the optional `web-react/session-gateway` subpath (`createSessionGatewayLane`). Server side, `stream`'s `stampReplaySeq` is what makes the cursor real — without it every reconnect refetches `fromSeq=0`. Transport recovery only: the helper never reduces, because the two lanes speak different vocabularies |
