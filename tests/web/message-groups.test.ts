...
test('Successive updates retain one attribution until a real user message', () => {
  ...
  assert.deepEqual(r.map(x => x.isContinuation), [false, true, false, false]); assert.equal(r[0].groupId, r[1].groupId); assert.notEqual(r[1].groupId, r[3].groupId)
})
test('Tool and notice rows do not restart assistant attribution', () => {
  ...
  assert.equal(r[3].isContinuation, true); assert(!Object.hasOwn(r[1], 'isContinuation'))
})
test('A working/streaming continuation shares the preceding assistant group', () => {
  ...
  assert(r.slice(1).every(x => x.isContinuation)); assert.equal(r[2].groupId, 'a')
})
test('The first visible message is labeled after history pagination', () => {
  const r = groupConversationMessages([assistant('later'), assistant('latest')]); assert.equal(r[0].isContinuation, false)
})
...
test('Distinct conversations reset grouping even without a user row', () => {
  ...
  assert.equal(r[1].isContinuation, false)
})
test('A non-message tool role cannot impersonate a user and reset the group', () => {
  const r = groupConversationMessages([assistant('a'), { id: 't', kind: 'tool', role: 'user' }, assistant('b')]); assert.equal(r[2].isContinuation, true)
})
test('Stored objects, text, timestamps and identities are not merged or mutated', () => {
  ...
  assert.equal(JSON.stringify(input), before); assert.equal(r.length, 2); assert.equal(r[0].content, 'a'); assert.equal(r[1].id, 'b'); assert('createdAt' in r[0]); assert.equal(r[0].createdAt, 'yesterday')
})
test('Empty input and missing IDs are supported without persistent global state', () => {
  assert.deepEqual(groupConversationMessages(), [])
  assert.equal(groupConversationMessages([{ role: 'assistant' }])[0].groupId, 'assistant-0')
  assert.equal(groupConversationMessages([assistant('again')])[0].isContinuation, false)
})
