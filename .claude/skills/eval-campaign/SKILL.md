---
name: eval-campaign
description: Wire product measurement and self-improvement to current Agent Eval campaigns, official optimization methods, and explicit release decisions.
---

# Wire Eval Campaigns

Use `@tangle-network/agent-app/eval-campaign` as the product-facing import.
It composes Agent Eval `0.129.0`; do not recreate execution, scoring, data splitting, or optimization.
...
7. Promote only when the returned release decision says to ship.
...
- Use `measurement-validation` when the metric has not yet separated known-good and known-bad cases.
