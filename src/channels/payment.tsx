...
import { ChannelState, ChannelFailure, buttonClass, panelClass } from './ui'
...
export function LinePayPage({ lineId, className = '' }: { lineId: string; className?: string }) {
  ...
  useEffect(() => setConsent(false), [lineId])
  return <section className={`${panelClass} ${className}`} aria-label="Line subscription">
    ...
    {!payment.available ? <p>Payments are unavailable for this line.</p> : <ChannelState resource={payment.resource} empty="No plan is available.">{value => <>
      ...
      {value.allowance.tier === 'paid' ? <p role="status">Your paid allowance is active.</p> : value.allowance.decision === 'paywall' && value.plan.paid ? <>
        ...
        <label className="flex items-start gap-2"><input type="checkbox" checked={consent} onChange={event => setConsent(event.target.checked)} />I accept these subscription terms.</label>
        <button type="button" className={buttonClass} disabled={!consent || !value.terms.trim() || payment.state.status === 'pending'} onClick={() => void payment.run({ consent })}>Continue to checkout</button>
    ...
    {payment.state.status === 'succeeded' && <a className="underline" href={payment.state.value} rel="noopener noreferrer">Open secure checkout</a>}
