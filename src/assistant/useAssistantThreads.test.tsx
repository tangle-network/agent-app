...
describe("useAssistantThreads", () => {
  ...
  // `fetchThreads` answers a non-ok response / unusable body / thrown request
  // with null. That used to settle as `loaded: true` over an empty list, which
  // is byte-identical to a user who has never chatted.
  it("reports a failed load instead of settling as an empty list", async () => {
    const { fetchThreads, wrapper } = setup();
    fetchThreads.mockResolvedValue(null);
    const { result } = renderHook(() => useAssistantThreads("userA"), {
      wrapper,
    });
  ...
  it("keeps the last good list and still reports the failure", async () => {
    const { fetchThreads, wrapper } = setup();
    fetchThreads.mockResolvedValueOnce([thread("t1")]).mockResolvedValueOnce(null);
    const { result } = renderHook(() => useAssistantThreads("userA"), {
      wrapper,
    });
  ...
  it("clears the failure once a retry succeeds", async () => {
    const { fetchThreads, wrapper } = setup();
    fetchThreads.mockResolvedValueOnce(null).mockResolvedValueOnce([thread("t1")]);
    const { result } = renderHook(() => useAssistantThreads("userA"), {
      wrapper,
    });
  ...
  it("reports a thrown request as a failed load", async () => {
    const { fetchThreads, wrapper } = setup();
    fetchThreads.mockRejectedValue(new Error("boom"));
    const { result } = renderHook(() => useAssistantThreads("userA"), {
      wrapper,
    });
  ...
  it("does not carry one account's failure into another's", async () => {
    const { fetchThreads, wrapper } = setup();
    fetchThreads.mockResolvedValue(null);
    const { result, rerender } = renderHook(
      ({ uid }: { uid: string }) => useAssistantThreads(uid),
      { wrapper, initialProps: { uid: "userA" } },
    );
  ...
  it("treats a signed-out user as an empty list, not a failure", async () => {
    const { wrapper } = setup();
    const { result } = renderHook(() => useAssistantThreads(null), { wrapper });
