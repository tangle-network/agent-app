...
describe("streamChat", () => {
  ...
  it("preserves a requirement's prerequisite, and drops a malformed one", async () => {
    // The card WITHHOLDS the App install on this field, so dropping it here
    // would put the install back in front of the connection it is claimed
    // through — the same class of loss `kind` suffered above. A malformed
    // reference is dropped whole: matched by (provider, kind), a half-read one
    // would withhold against the wrong row, or none.
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
