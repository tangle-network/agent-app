 ...
 *
 * The harness taxonomy comes directly from `@tangle-network/agent-interface`,
 * the shared contract already consumed by sandbox-ui and the CLI bridges. The
 * consumer owns storage — which harness a workspace defaults to, which one a
 * session locked — and maps the resolved value onto the SDK's `backend.type`.
 *
...
/** The known coding-agent backends, copied from the shared runtime schema so
 * callers cannot mutate the schema's own options array. */
export const KNOWN_HARNESSES: readonly HarnessType[] = Object.freeze([
  ...harnessTypeSchema.options,
])
 ...
 *
 * `Harness` is an alias of agent-interface's `HarnessType`, so the server guard,
 * the model compatibility policy, and every shared picker accept exactly the
 * same values without casts or parallel lists.
 */
