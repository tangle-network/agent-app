...
describe('this package audits itself', () => {
  ...
  it('supports the verified Runtime line without claiming the next one', async () => {
    ...
    // and 0.273.5 the first that admits 0.53.
    ...
    expect(satisfiesRange('0.273.999', range!)).toBe(true)
    expect(satisfiesRange('0.274.0', range!)).toBe(false)
  })
  ...
  // rows below are the ones the installed Runtime (0.273.5, Eval >=0.188.0) admits.
