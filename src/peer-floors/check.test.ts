...
describe('this package audits itself', () => {
  it('declares support for the current Interface line without claiming the next one', async () => {
    ...
    expect(satisfiesRange('0.46.0', range!)).toBe(false)
    expect(satisfiesRange('0.46.1', range!)).toBe(true)
    expect(satisfiesRange('0.46.9', range!)).toBe(true)
    expect(satisfiesRange('0.47.0', range!)).toBe(false)
  })
