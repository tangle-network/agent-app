...
- **Resumable turns** — buffer a streaming turn so a dropped tab or Worker restart loses nothing; a reconnecting client replays the tail by sequence number. Works whether you own the stream (`pumpBufferedTurn`) or the engine does (`createBufferedTurnTap` over `handleChatTurn`'s `onEvent`). See [`examples/resumable-turns.md`](./examples/resumable-turns.md).
