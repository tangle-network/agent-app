...
- **A spinner with nothing to say.** Six `animate-spin` uses; a spinner without a label naming what is being waited on communicates only "something", which is the one thing the reader already knew.
