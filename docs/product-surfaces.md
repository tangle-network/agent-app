...
**Rejected — the second one, and it is the reference implementation** (measured 2026-08-04).
`agent-app-uplift src/web-react/record-grid.tsx:929` reads `const basis = source.basis ?? 'source'`, over a `RecordGridSourceBasis` of `source | confirmed | derived`, whose `source` renders the tooltip "Extracted from a source document" (`:192`).
So a cell whose caller did not state a basis — the default path, the one every first integration takes — tells a CPA the figure was read out of a document.
There is no fourth value the caller could have passed to say the agent simply asserted it.
The union is the defect, and the missing default makes it fire silently: **an omission renders as a factual claim about a document that may not exist.**
Three lanes reached for this concept in the same week and produced three vocabularies — `source|confirmed|derived` here, `extracted|entered|computed|asserted` in `src/web-react/provenance-model.ts`, and an invented `source|confirmed|derived` in the first draft of this very document.
That is the four-verticals failure reproducing inside one branch, which is the argument for a canonical vocabulary being written down and tested rather than agreed.
...
- **A spinner with nothing to say.** Five `animate-spin` uses; a spinner without a label naming what is being waited on communicates only "something", which is the one thing the reader already knew.
