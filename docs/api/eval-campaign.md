...
```ts
<TScenario extends Scenario$1, TArtifact>(opts: SelfImproveOptions<TScenario, TArtifact>) => Promise<SelfImproveResult<…
```

### `SelfImproveBudget`

`interface` — Run one complete improvement job.
