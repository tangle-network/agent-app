...
```ts
(document: SceneDocument) => Map<string, { pageId: string; elementId: string; kind: "text" | "image" | "rect" | "ellips…
```
...
```ts
readonly ("text" | "image" | "rect" | "ellipse" | "line" | "video" | "group")[]
```
...
```ts
(slotName: string, elementKind: "text" | "image" | "rect" | "ellipse" | "line" | "video" | "group", value: string) => v…
```
