...
```ts
(document: SceneDocument) => Map<string, { pageId: string; elementId: string; kind: "text" | "image" | "rect" | "line"…
```
...
```ts
readonly ("text" | "image" | "rect" | "line" | "ellipse" | "video" | "group")[]
```
...
```ts
(slotName: string, elementKind: "text" | "image" | "rect" | "line" | "ellipse" | "video" | "group", value: string) => v…
```
