...
`class` — Thrown by {@link requireTransportableModel} when a model that was EXPLICITLY requested — via a per-turn override or a configured `provider.modelName` — cannot be transported to the sandbox platform.
